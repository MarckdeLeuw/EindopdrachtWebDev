...
})({"gZM3h":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "8c6fc083b45875fe";
...
},{}],"fPG0g":[function(require,module,exports) {
...
/*====================================
1.0  DEFINE STARTVRAAG
====================================*/ let questionStart = _dataInput.questions[0].questionInput;
...
====================================*/ function responseQuestion(objectQuestion) {
    for(let i1 = 0; i1 < objectQuestion.length; i1++)if (objectQuestion[i1].questionInput === questionStart) {
        const possibleAnswers = objectQuestion[i1].possibleAnswer;
        //console.log(possibleAnswers);
        return possibleAnswers;
    }
}
let possibleAnswer = responseQuestion(_dataInput.questions);
/*====================================
3.0 ANTWOORDEN IN RADIOBOX ZETTEN
====================================*/ /*====================================
...
for(let i = 0; i < possibleAnswer.length; i++)if (i === 0) listAnswers.innerHTML += `${buttonItemChecked(possibleAnswer[i])}`;
else listAnswers.innerHTML += `${buttonItem(possibleAnswer[i])}`;
/*====================================================================
4.0 ACTIES AAN BUTTON VERBINDEN
======================================================================*/ /*====================================================================
4.1 VERVOLGVRAGEN
...
function grabNew() {
    ...
    let question1 = document.forms[0];
    let newQuestion = '';
    let txt = '';
    for(let j = 0; j < question1.length; j++)if (question1[j].checked) txt = txt + question1[j].value + '';
    //geeft de vervolgvraag
    for(let j1 = 0; j1 < _dataInput.questions.length; j1++){
        for(let k = 0; k < _dataInput.questions[j1].possibleAnswer.length; k++)if (_dataInput.questions[j1].possibleAnswer[k] === txt) {
            newQuestion = _dataInput.questions[j1].nextQuestion[k];
    ...
    for(let j2 = 0; j2 < _dataInput.questions.length; j2++)if (_dataInput.questions[j2].questionInput === newQuestion) {
        let newPossibleAnswers = _dataInput.questions[j2].possibleAnswer;
        // console.log(questions[j].lastQuestion);
        if (_dataInput.questions[j2].lastQuestion === true) {
        ...
        for(let k = 0; k < newPossibleAnswers.length; k++)if (k === 0) listAnswers.innerHTML += `${buttonItemChecked(newPossibleAnswers[k])}`;
        else listAnswers.innerHTML += `${buttonItem(newPossibleAnswers[k])}`;
    }
}
/*======================================================
4.2 NIEUWE GET RECIPE BUTTON OM OUDE NEXT QUESTION BUTTON TE VERVANGEN
...
========================================================*/ function findFilters() {
    //zoekt waarde radio button
    let question2 = document.forms[0];
    let newQuestion = '';
    let txt = '';
    for(let j = 0; j < question2.length; j++)if (question2[j].checked) txt = txt + question2[j].value + '';
    for(let j3 = 0; j3 < _dataInput.questions.length; j3++){
        for(let k = 0; k < _dataInput.questions[j3].possibleAnswer.length; k++)if (_dataInput.questions[j3].possibleAnswer[k] === txt) {
            let filter = _dataInput.questions[j3].filters[k];
            document.getElementById('question').textContent = newQuestion;
            // console.log(filter)
            // let op meteen voor api leesbaar maken
            cuisine = _dataInput.questions[j3].filters[k].cuisine;
            type = _dataInput.questions[j3].filters[k].type;
            intolerances = _dataInput.questions[j3].filters[k].intolerances;
            diet = _dataInput.questions[j3].filters[k].diet;
            //let maxReadyTime = "";
            //
            query = _dataInput.questions[j3].filters[k].query;
            includeIngredients = _dataInput.questions[j3].filters[k].includeIngredients;
        }
    }
}
/*=====================================================
...
=====================================================*/ function createRecipeList(recipes) {
    ...
    for(let i2 = 0; i2 < recipes.length; i2++)button(recipes[i2].id.toString()).addEventListener('click', clickRecipe);
}
/*=====================================================
5.4 RECIPE DETAIL
...
function clickRecipe(e) {
    const currentValue = e;
    for(let i3 = 0; i3 < allRecipes.length; i3++){
        let recipe = document.getElementById(`place-ref-${allRecipes[i3].id}`);
        recipe.replaceChildren();
        if (currentValue.target.id == allRecipes[i3].id) {
            recipeDetails.id = allRecipes[i3].id;
            recipeDetails.title = allRecipes[i3].title;
            recipeDetails.image = allRecipes[i3].image;
...
},{"./dataInput":"fMq9m","axios":"jo6P5","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fMq9m":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "inventory", ()=>inventory
);
parcelHelpers.export(exports, "mainQuestions", ()=>mainQuestions
);
...
const questions = [
    ...
    {
        questionInput: "If you have a cold, it is wise to eat the following things.\nVitamin C can shorten the period of illness. Eat as many fruits and vegetables as possible with a lot of vitamin C. Think, for example, of rosehip, bell pepper, chicory, kiwi and broccoli.\n\nA warm soup ensures that you often feel better immediately. It softens your throat and 'opens' your nose again. Eat nice chicken soup with lots of vegetables!\n\nProteins are an important building material for your body, for example for the antibodies and enzymes of the immune system. Proteins can be found in fish, eggs, meat, beans and legumes.\n\nChili peppers contain the substance capsaicin. Research has shown that this substance supports your immune system. Do we go for the soup or do you feel more like something else?",
        lastQuestion: true,
        possibleAnswer: [
    ...
    {
        questionInput: "First of all drink a lot of water!\n\nEat eggs. These are full of amino acids such as cysteine and taurine. Also take extra potassium in the form of bananas, kiwis or leafy vegetables. Ginger is also great. Still in the mood for something fat? Go for salmon. The fish oil and B12 help the stomach to break down alcohol. Do you want to go for the wise choice?",
        lastQuestion: true,
        possibleAnswer: [
...
},{"./../utils":"5By4s"}]},["gZM3h","fPG0g"], "fPG0g", "parcelRequire6362")

//# sourceMappingURL=questions.b45875fe.js.map
