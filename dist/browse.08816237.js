...
})({"bGbYd":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "24593d4108816237";
...
},{}],"c6Oi2":[function(require,module,exports) {
...
====================================================*/ function addToSelected(list, refPlace) {
    ...
    for(let i2 = 0; i2 < list.length; i2++){
        const listItem1 = document.createElement('h6');
        listItem1.textContent = `${list[i2]}`;
        //`${list[i]}`
        refPlace.appendChild(listItem1);
    }
}
/*=====================================================
...
for(let i = 0; i < _dataInput.filters.length; i++){
    listAll.innerHTML += `${categoryItem(_dataInput.filters[i])}`;
    const productList = document.getElementById(_dataInput.filters[i].category);
    for(let j = 0; j < _dataInput.filters[i].products.length; j++)productList.innerHTML += `${listItem(_dataInput.filters[i].products[j])}`;
}
/*=====================================================
...
for(let i1 = 0; i1 < _dataInput.filters.length; i1++)for(let j = 0; j < _dataInput.filters[i1].products.length; j++)button(_dataInput.filters[i1].products[j]).addEventListener('click', handleInputChange);
/*===================================================
...
====================================================*/ function handleInputChange(e) {
    ...
    for(let i3 = 0; i3 < _dataInput.filters.length; i3++){
        for(let j1 = 0; j1 < _dataInput.filters[i3].products.length; j1++)if (currentValue.target.name === _dataInput.filters[i3].products[j1]) // console.log('selected '+ filters[i].category);
        switch(_dataInput.filters[i3].category){
            case _dataInput.filters[0].category:
                ...
                } else //als element er al in zit, moet hij eruit gehaald worden.
                if (selectedCuisinesTypes.includes(currentValue.target.name)) {
                ...
                break;
            case _dataInput.filters[1].category:
                if (currentValue.target.checked === true) {
                    // stopt waarde in array als deze er nog niet inzit, anders kunnen er meerdere zelfde items inkomen
                ...
                break;
            case _dataInput.filters[3].category:
                if (currentValue.target.checked === true) {
                    // stopt waarde in array als deze er nog niet inzit, anders kunnen er meerdere zelfde items inkomen
                ...
                break;
            case _dataInput.filters[2].category:
                if (currentValue.target.checked === true) {
                    // stopt waarde in array als deze er nog niet inzit, anders kunnen er meerdere zelfde items inkomen
...
formTime.addEventListener('submit', (e)=>{
    ...
    selectedMaxReadyTime = [
        `${parseInt(inputMaxReadyTime.value)}`
    ];
    ...
    try {
        const response = await _axiosDefault.default.get("https://api.spoonacular.com/recipes/complexSearch", {
            params: {
                ...
                number: 1 //hoeveel recepten teruggegven
            },
            headers: {
...
=====================================================*/ function createRecipeList(recipes) {
    ...
    function button1(recipeId) {
        return document.getElementById(recipeId);
    }
    for(let i4 = 0; i4 < recipes.length; i4++)button1(recipes[i4].id.toString()).addEventListener('click', clickRecipe);
}
/*=====================================================
4.4 RECIPE DETAIL
...
function clickRecipe(e) {
    ...
    for(let i5 = 0; i5 < allRecipes.length; i5++){
        let recipe = document.getElementById(`place-ref-${allRecipes[i5].id}`);
        recipe.replaceChildren();
        // console.log(currentValue.target.id);
        // console.log(allRecipes[0].id);
        if (currentValue.target.id == allRecipes[i5].id) {
            recipeDetails.id = allRecipes[i5].id;
            recipeDetails.title = allRecipes[i5].title;
            recipeDetails.image = allRecipes[i5].image;
...
},{"./../utils":"5By4s"}]},["bGbYd","c6Oi2"], "c6Oi2", "parcelRequire6362")

//# sourceMappingURL=browse.08816237.js.map
